...
[전략 개요]
거래대금 상위 종목을 실시간 모니터링하여 급등하는 종목을 감지

[매수 신호]
- 전일 대비 상승률 >= 설정값 (기본: 5%)
- 거래량 >= 평균 거래량 x 배수 (기본: 2배)
- 쿨다운 시간 경과
...
class SurgeStrategy:
    ...
    def __init__(
        self,
        broker,
        surge_callback: Optional[Callable] = None,
        ...
        Args:
            broker: 브로커 어댑터
            surge_callback: 급등 감지 시 호출할 콜백
            candidate_count: 후보 종목 수
            min_change_rate: 최소 상승률 (%)
            min_volume_ratio: 최소 거래량 비율
            cooldown_minutes: 재감지 대기시간 (분)
        """
        self.broker = broker
    ...
    def initialize(self) -> bool:
        """초기화 및 후보군 로드"""
        try:
            logger.info("급등주 후보군 로드 중...")
            
            # 거래대금 상위 종목 조회
            top_stocks = self.broker.get_top_traded_stocks(self.candidate_count)
            ...
            logger.success(f"급등주 후보군 로드 완료: {len(self.candidates)}개 종목")
            
            # 실시간 시세 등록
            candidate_codes = list(self.candidates.keys())
            self.broker.register_realtime(candidate_codes)
            
            self.is_initialized = True
            return True
            
        except Exception as e:
            logger.error(f"급등주 전략 초기화 중 오류: {e}")
            return False
    
    def start_monitoring(self):
        """실시간 모니터링 시작"""
        if not self.is_initialized:
            logger.error("초기화되지 않았습니다. initialize()를 먼저 호출하세요.")
            return
        
        self.is_monitoring = True
        logger.success(f"급등주 모니터링 시작: {len(self.candidates)}개 종목")
    
    def stop_monitoring(self):
        """모니터링 중지"""
        self.is_monitoring = False
        logger.info("급등주 모니터링 중지")
